    ...
    public class ParaderosController
    {
        ...
        public string Grabar(string periodo, SJ_Paraderos paradero)
        {
            ...
            using (BDAsistenciaDataContext modelo = new BDAsistenciaDataContext(cnx))
            {
                ...
                resultadoConsulta = modelo.SJ_Paraderos.Where(x => x.IdParadero == paradero.IdParadero).ToList();
        ...
        public string Anular(string periodo, SJ_Paraderos paradero)
        {
            ...
            {
                modelo.CommandTimeout = 9999900;
                resultadoConsulta = modelo.SJ_Paraderos.Where(x => x.IdParadero == paradero.IdParadero && x.IdEmpresa == paradero.IdEmpresa).ToList();
                int nuevoCodigo = Convert.ToInt32(modelo.SJ_Paraderos.Max(x => x.IdParadero)) + 1;
