...
namespace Transportista.Negocios
{
    public class TransportistaNeg
    {
        ...
        public Int32 RegistrarTransportista(SJ_RHTransportista ObjetoTransportista,
            List<SJ_RHTransportistaChofer> listadoChoferes, List<SJ_RHTransportistaContrato> listadoContratos,
            List<SJ_RHTransportistaChofer> ListaEliminadosChoferes, List<SJ_RHTransportistaContrato> ListaEliminadosContratos,
            List<SJ_RHTransportistaRuta> ListaEliminadosRuta, List<SJ_RHTransportistaRuta> ListadoRutas)
        {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (listadoChoferes.ToList().Count > 0)
                            {
                                foreach (SJ_RHTransportistaChofer item in listadoChoferes)
                                {
                                    if (Modelo.SJ_RHTransportistaChofer.Where(x => x.Id == CodigoRegistro && x.Item == item.Item).ToList().Count == 1)
                                    {
                                        #region Actualizar Chofer()
                                        oChofer = new SJ_RHTransportistaChofer();
                                        oChofer = Modelo.SJ_RHTransportistaChofer.Where(x => x.Id == CodigoRegistro && x.Item == item.Item).Single();
                                        oChofer.DNI = item.DNI;
                                        oChofer.Nombres = item.Nombres;
                                        oChofer.TipoLicencia = item.TipoLicencia;
                                        oChofer.EsCapacitado = item.EsCapacitado;
        ...
        public void EliminarChofer(SJ_RHTransportistaChofer Objeto)
        {
            using (TransactionScope Scope = new TransactionScope())
            {
                ...
                using (SJ_RHFacturacionTransportistaDataContext Modelo = new SJ_RHFacturacionTransportistaDataContext(cnx))
                {
                    ...
                    if (Modelo.SJ_RHTransportistaChofer.Where(x => x.Id == Objeto.Id && x.Item.ToString().Trim() == Objeto.Item.ToString().Trim()).ToList().Count == 1)
                    {
                        Chofer = Modelo.SJ_RHTransportistaChofer.Where(x => x.Id == Objeto.Id && x.Item.ToString().Trim() == Objeto.Item.ToString().Trim()).Single();
                        try
                        {
